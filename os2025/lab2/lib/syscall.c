...
char getChar(){ // 对应SYS_READ STD_IN
	...
	while(a != '\n'){
		res = (char)(a & 0xff);
		a = syscall(SYS_READ, STD_IN, 0, 0, 0, 0);	// it seems that we don't need more paramter.
	} 
	return res;
	// OVER TODO
}
...
void getStr(char *str, int size){ // 对应SYS_READ STD_STR
	...
	// waiting : esi = 1
	// edi = begin_Tail
	beginTail = syscall(SYS_READ, STD_STR, (uint32_t)str, (uint32_t)size, 0, 0);
	// failed = 0
	// succeed = 1
	int get_succeed = 0;
	while(get_succeed == 0){
		get_succeed = syscall(SYS_READ, STD_STR, (uint32_t)str, (uint32_t)size, 1, beginTail);
	}
	return;
	// OVER TODO
...
void printf(const char *format,...){
	...
	// DO
	// maybe it is not useless in other implements
